...
wsUtil = {
	 ...
	 *
	 * to disable default navigation, deselect "Main Menu" in theme options and use menu widgets instead
	 * to disable modals or reveals, just don't put any widgets in those regions
	...
	init : function() {
		...
		// validate any forms
		if (jQuery('form').validate && TLDRoptions.validateForms) {
		...
		// fix footer
		if (TLDRoptions.fixFooter) {
			wsUtil.fixFooter();
		}

		// shorten links
		if (TLDRoptions.shortenLinks) {
			wsUtil.shortenLinks();
		}

		// external links
		if (TLDRoptions.externalLinks) {
			wsUtil.prepareExternalLinks(TLDRoptions.externalLinksExceptions);
		}

		// section navigation
		if (TLDRoptions.sectionNavigationSelector) {
			wsUtil.prepareSectionNavigation(TLDRoptions.sectionNavigationSelector, TLDRoptions.sectionNavigationPadding);
		}
	},

	// 2.1.2 javascript for mobile and drop-down navigation
	prepareNavigation : function(container) {
		jQuery('#navigation .main-menu > ul').append('<li class="menu-dismiss fa"></li>');
		jQuery('#navigation .main-menu ul li ul').before('<span class="menu-toggle closed fa" title="Open submenu"></span>');
		jQuery('#navigation .menu-toggle').click(function(){
			if (jQuery(this).hasClass('closed')) {
				jQuery(this).removeClass('closed').addClass('open').attr('title','Close submenu');
				jQuery(this).siblings('ul').addClass('open');
			} else {
				jQuery(this).removeClass('open').addClass('closed').attr('title','Open submenu');
				jQuery(this).siblings('ul').removeClass('open');
			}
		});
		jQuery('#navigation li.page_item_has_children > .nolink').click(function(){
		...
		if (jQuery('.mobile-style-default #navigation .main-menu > ul').length) {
			var menuHammer = new Hammer(jQuery('.mobile-style-default #navigation .main-menu > ul')[0]);
			menuHammer.on('swipeleft', function(event){
				if (jQuery(window).width() < 768) { jQuery('#navigation .main-menu > ul').removeClass('open'); }
			});
		}
		...
		jQuery(document).keyup(function(event){
			if ( (event.keyCode == 27) && (jQuery(window).width() < 768) ) { jQuery('#navigation .main-menu > ul').removeClass('open'); }
		});
	},
	...
	prepareModalsandReveals : function() {
		...
		jQuery('a, .modal-trigger, .reveal-trigger').click(function(event){
			...
			if ((href.substr(0,11) == '/user/login') && (jQuery('#modals #user-login-form').length || jQuery('.reveal #user-login-form').length)) {
				...
				$target = jQuery('#modals #user-login-form').closest('.widget');
				if (!$target.length) { $target = jQuery('.reveal #user-login-form').closest('.widget'); }
			...
			if (!$target.hasClass('widget')) { return true; }
	...
	openModal : function(sel, options) {
		...
		if (!$el.closest('#modals').length || !$el.hasClass('widget')) { return; }
		if ($el.hasClass('open')) { return; }

		jQuery('#modals .widget').hide();
	...
	closeModal : function() {
		...
		window.setTimeout(function(){ jQuery('#modals .widget').hide().removeClass('open'); jQuery('#modals-wrapper').hide(); }, 1000);
	},
...
jQuery(window).load(function(){
	if (TLDRoptions.fixFooter) { wsUtil.fixFooter(); }
	if (TLDRoptions.shortenLinks) { wsUtil.shortenLinks(); }
});
